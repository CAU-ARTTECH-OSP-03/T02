...
class Startwindow(QWidget):  # A
    ...
    def initUI(self):
        ...
        label1.move(350, 200)
        label1.setFont(QtGui.QFont("맑은고딕",10))  # 한글 폰트 깨지는 문제 발생 -> 시각적으로 보기 좋은 폰트로 수정
        label1.setStyleSheet("Color : white")
        label2 = QLabel('유미의 세포들 심리테스트에', self)
        label2.move(360, 250)
        label2.setFont(QtGui.QFont("맑은고딕", 10))
        label2.setStyleSheet("Color : white")
        label3 = QLabel('오신 걸 환영합니다!', self)
        label3.move(390, 300)
        label3.setFont(QtGui.QFont("맑은고딕", 10))
        label3.setStyleSheet("Color : white")
        label4 = QLabel('아래 버튼을 눌러주세요', self)
        label4.move(370, 350)
        label4.setFont(QtGui.QFont("맑은고딕", 10))
...
class Result(QWidget):
    ...
    def loadcell(self):
        ...
        if self.result == 7:
            pixmap1.load('./cell/istj.png') # 세포 이미지 위치, main.py 아래에 cell 폴더 만들어 .jfif 형식의 이미지 저장
...
class Etc(QWidget): # 게임 설명
    ...
    def initUI(self):
        ...
        bgImage = QImage('bg.png')
        size = bgImage.scaled(QSize(1000, 800))
        palette = QPalette()
        palette.setBrush(10, QBrush(size))
    ...
    def script(self):
        ...
        pixmap1 = QPixmap()
        img1 = pixmap1.scaledToWidth(100)
        pixmap2 = QPixmap()
        img2 = pixmap2.scaledToWidth(100)
        pixmap3 = QPixmap()
        img3 = pixmap3.scaledToWidth(100)
        cell_txt = QLabel()
        self.cut = mbti_result()
        print(self.cut)

         #mbti 결과 반환 int = result


        if self.cut == 7:
            img1.load('./icon/istj.png')# 세포 이미지 위치, main.py 아래에 cell 폴더 만들어 .jfif 형식의 이미지 저장
            img2.load('./icon/istj.png')
            img3.load('./icon/istj.png')
            cell_txt.setText("실제 사실에 대해 정확하고 체계적으로 기억하며, 일처리에 있어도 신충하고 책임감 있는 당신!\n\n"
                             "변화보단 반복을 좋아하는 당신의 머리 속의 가장 중요한 세포는\n\n"
                             "바로 다른 이들에게 항상완벽한 모습으로 예의를 지키는 “예의세포” 입니다~!")
        elif self.cut == 1:
            img1.load('./cell/infj.png')
            img2.load('./cell/infj.png')
            img3.load('./cell/infj.png')
            cell_txt.setText("항상 상대방을 위해 배려하고 진심 어린 공감을 해주는 당신!\n\n"
                             "하지만 평상시에 생각이 너무 많아 걱정이 많은 당신의 머리 속의 가장 중요한 세포는 바로\n\n"
                             "안 좋은 상황을 미리 걱정하고 세포들에게 대비할 수 있도록 도움을 주는 “불안 세포”입니다~!")
        elif self.cut == 3:
            img1.load('./cell/intj.png')
            img2.load('./cell/intj.png')
            img3.load('./cell/intj.png')
            cell_txt.setText("계획적이고 효율적이면 혼자 있는 것을 좋아하는 당신!\n\n"
                             "분석력 또한 매우 뛰어난 편인 당신의머리 속의 가장 중요한 세포는 바로\n\n"
                             "상황을 분석하고 추리해내는 능력이 뛰어나지만, 잘못 분석하는 경우가 있어 허당미가 있는 “명탐정 세포” 입니다~!")
        elif self.cut == 6:
            img1.load('./cell/istp.png')
            img2.load('./cell/istp.png')
            img3.load('./cell/istp.png')
            cell_txt.setText("호불호가 강하고 자기주장도 강한 당신!\n\n"
                             "내성적이면서도 냉철한 이성주의자인 당신의 머리 속의 가장 중요한 세포는 바로\n\n"
                             "이유 없이 화내진 않지만, 할 말을 다 못하면 잠재되어 있던 분노가 격하게 치밀어 오르는 “히스테리우스” 입니다~!")
        elif self.cut == 4:
